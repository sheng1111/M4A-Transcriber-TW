...
VoiceScribe v2.4.3 converts recorded audio into a raw transcript and, unless transcript-only mode is selected, a faithful translated result. The default target is Taiwan Traditional Chinese (`zh-TW`), and users may select another BCP 47 language tag. `app.py` and `gui_app.py` are entry points; reusable code lives in `transcriber/`.
